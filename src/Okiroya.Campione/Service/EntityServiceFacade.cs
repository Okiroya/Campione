...
namespace Okiroya.Campione.Service
{
    ...
    public static class EntityServiceFacade
    {
        ...
        /// <param name="sortable">Флаг добавления параметров сортировки</param>
        /// <returns></returns>
        public static async Task<PagedCollection<IEntityObject<int>>> ExecutePagedQueryAsync(string commandName, IDictionary<string, object> parameters, CancellationToken cancellationToken, int page = 1, int pageSize = 0, string sortBy = null, bool paged = true, bool sortable = true)
        {
            var isAscendingSort = !(sortBy ?? string.Empty).EndsWith(ParametersExtensions.RevParamPostfixName, StringComparison.OrdinalIgnoreCase);
            ...
            var data = await ExecuteQueryAsync(commandName, executedParameters, cancellationToken).ConfigureAwait(false);
